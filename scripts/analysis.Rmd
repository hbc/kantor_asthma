...
What if we focus on just the 
non-rhinovirus positive samples?

```{r no-rhinovirus-analysis}
design = model.matrix(~ 0 + WBC + Hct_pct + Hgb +
    Plt + AEC + ANC + ALC + Neut_pct + Baso_pct + virus_status,
    data=samples_norm)
y = DGEList(counts=counts)
y = calcNormFactors(y)
v = voom(y, design, plot=TRUE)
corfit = duplicateCorrelation(v, design, block=samples$patient)
cm = makeContrasts(
    rhinovirus=virus_statuspositive_exacerbation-virus_statusnegative_exacerbation,
    exacerbation=virus_statusnegative_exacerbation-virus_statusnegative_baseline,
    rhinovirus_asthma=virus_statuspositive_exacerbation-virus_statusnegative_baseline,
    levels=design)
fit = lmFit(v, design, block=samples$patient, correlation=corfit$consensus)
fit2 = contrasts.fit(fit, cm)
fit2 = eBayes(fit2)
...
```{r controlled-de}
design = model.matrix(~ 0 + rhinovirus + WBC + AEC + ANC + ALC +
    status, data=samples_norm)
cm = makeContrasts(
    rhinovirus=rhinoviruspositive-statusexacerbation,
    exacerbation=statusexacerbation,
    levels=design)
y = DGEList(counts=counts)
y = calcNormFactors(y)
v = voom(y, design, plot=TRUE)
